...
public class EnqueueApi {
    ...
    private RecordChangedMessages getTaskQueueMessage() {
        try {
            ...
            if (message == null) {
                throw new AppException(HttpStatus.BAD_REQUEST.value(), errorText, "message object not found", "'message' object not found in PubSub message");
            }
            ...
            if(Strings.isNullOrEmpty(payload)) {
                throw new AppException(HttpStatus.BAD_REQUEST.value(), errorText, "message data not found", "'message.data' not found in PubSub message");
            }
            ...
            if (attributes == null || attributes.size() == 0) {
                throw new AppException(HttpStatus.BAD_REQUEST.value(), errorText, "attribute map not found", String.format("PubSub message: %s", recordChangedMessages));
            }
            ...
            if (recordChangedMessages.missingAccountId()) {
                throw new AppException(HttpStatus.BAD_REQUEST.value(), "Invalid tenant", "tenant-id missing", String.format("PubSub message: %s", recordChangedMessages));
            }
